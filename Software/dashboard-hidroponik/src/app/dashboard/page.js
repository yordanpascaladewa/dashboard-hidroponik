...
import { 
  ...
  Menu, X // <-- Gua tambahin icon Menu dan X di sini
} from 'lucide-react';
...
function IconButton({ icon, hiddenOnMobile }) {
  return (
    <button className={`bg-white border border-[#bbcabf]/30 text-[#3c4a42] hover:bg-[#f2f4f6] p-2 rounded-full transition-colors active:scale-95 shadow-sm ${hiddenOnMobile ? 'hidden' : 'block'}`}>
      {icon}
    </button>
  );
}
